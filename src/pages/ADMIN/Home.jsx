...
//     const res = await api.get("/admin/institutions/home");
//     setData(data);
...
const AHome = () => {
  ...
  const fetchData = async () => {
    const res = await api.get("/admin/institutions/home");
    setData(res.data); // ✅ fix: use res.data instead of 'data'
  };
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-6">
        {stats.map((stat, idx) => (
          <motion.div
            ...
            className="bg-white p-5 rounded-xl shadow-sm border border-gray-200 flex items-center gap-4 transition"
          >
            <div className={`p-3 rounded-full ${stat.color}`}>
              <stat.icon size={24} />
            </div>
